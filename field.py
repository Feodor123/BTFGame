...
class GameField:
    def __init__(self, game, size: Vector):
        ...
        self.rooms = []
        self.roads = []
        self._tiles = [[Tile(TileType.Empty, have_sprite=True) for
                        y in range(self.size.y)] for x in range(self.size.x)]
        self.sprites = []
    ...
    def collide(self, rect: Rectangle):
        for x in range(math.floor(rect.pos.x),
                       math.floor(rect.pos.x + rect.size.x) + 1):
            for y in range(math.floor(rect.pos.y),
                           math.floor(rect.pos.y + rect.size.y) + 1):
    ...
    def on_field(self, v, indent=0):
        return indent <= v.x < self.size.x - indent and \
               indent <= v.y < self.size.y - indent
    ...
    def generate_rooms(self, game, min_distance=1):
        ...
        for i in range(1000):
            ...
            for x, y in [(x, y) for x in range(rect.pos.x - min_distance,
                                               rect.pos.x + rect.size.x +
                                               min_distance + 1)
                         for y in range(rect.pos.y - min_distance,
                                        rect.pos.y + rect.size.y +
                                        min_distance + 1)]:
                if self[x, y].tile_type != TileType.Empty and \
                        not self[x, y].obstacle:
                    break
            else:
                self.add_room(ToiletRoom(rect, TileType.Floor0,
                                         TileType.Wall0, TileType.Carpet0), 0)
                break
        for i in range(1000):
            rect = Rectangle.get_random(self.min_room_size,
                                        self.max_room_size, border)
            for x, y in [(x, y) for x in range(rect.pos.x - min_distance,
                                               rect.pos.x + rect.size.x +
                                               min_distance + 1)
                         for y in range(rect.pos.y - min_distance,
                ...
                self.add_room(Room(rect, TileType.Floor0, TileType.Wall0),
                              2 if len(self.rooms) > 0 else 0)
    ...
    def connect_room(self, room, connections_count):
        ...
        possible_in_doors = {v for r in self.rooms
                             if r != room for v in r.possible_doors()}
        while len(room.doors) < connections_count and \
                len(possible_out_doors) > 0:
            door = choice(tuple(possible_out_doors))
            path = self.dfs(door,
                            lambda v: self.on_field(v, 1) and
                            self[v.x, v.y].tile_type ==
                            TileType.Empty,
                            lambda v: v in possible_in_doors or
                            type(self[v.x, v.y].structure) == Road
                            and not self[v.x, v.y].obstacle)
            if path is None:
