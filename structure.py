...
class Room:
    ...
    def fill(self, field):
        for x in range(self.shape.pos.x,
                       self.shape.pos.x + self.shape.size.x):
            for y in range(self.shape.pos.y,
                           self.shape.pos.y + self.shape.size.y):
                field[x, y] = Tile(self.floor,
                                   have_sprite=True, structure=self)

    def border(self, field, replace: bool = False):
        for x in range(self.shape.pos.x - 1,
                       self.shape.pos.x + self.shape.size.x + 1):
            if replace or \
                    field[x, self.shape.pos.y - 1].tile_type == TileType.Empty:
                field[x, self.shape.pos.y - 1] = Tile(self.wall,
                                                      obstacle=True,
                                                      have_sprite=True,
                                                      structure=self)
            if replace or \
                    field[x, self.shape.pos.y + self.shape.size.y].tile_type \
                    == TileType.Empty:
                field[x,
                      self.shape.pos.y + self.shape.size.y] = \
                    Tile(self.wall,
                         obstacle=True, have_sprite=True, structure=self)
        for y in range(self.shape.pos.y, self.shape.pos.y + self.shape.size.y):
            if replace or field[self.shape.pos.x - 1, y].tile_type\
                    == TileType.Empty:
                field[self.shape.pos.x - 1, y] = Tile(self.wall, obstacle=True,
                                                      have_sprite=True,
                                                      structure=self)
            if replace or \
                    field[self.shape.pos.x + self.shape.size.x, y].tile_type\
                    == TileType.Empty:
                field[self.shape.pos.x + self.shape.size.x, y]\
                    = Tile(self.wall, obstacle=True, have_sprite=True,
                           structure=self)
...
class ToiletRoom(Room):
    def __init__(self, shape: Rectangle,
                 floor: TileType, wall: TileType, carpet: TileType):
    ...
    def border(self, field, replace: bool = False):
        ...
        for v in to_fill:
            if replace or field[v.x, v.y].tile_type == TileType.Empty:
                field[v.x, v.y] = Tile(self.wall, obstacle=True,
                                       have_sprite=True, structure=self)
